...
class SlpTxBuilder {
    companion object {
        ...
        @JvmStatic
        fun buildNftSendTx(nftTokenId: String, amount: Double, toAddress: String, slpAppKit: SlpAppKit, aesKey: KeyParameter?, allowUnconfirmed: Boolean): Single<Transaction> {
            return sendNftUtxoSelection(nftTokenId, amount, slpAppKit)
                .map {
                    val addrTo = SlpAddressFactory.create().getFromFormattedAddress(slpAppKit.wallet().params, toAddress).toCash()
                    // Add OP RETURN and receiver output
                    val req = SendRequest.createSlpTransaction(slpAppKit.wallet().params)
        ...
        @JvmStatic
        fun buildNftSendTx(nftTokenId: String, amount: Double, toAddress: String, slpAppKit: SlpBIP47AppKit, aesKey: KeyParameter?, allowUnconfirmed: Boolean): Single<Transaction> {
            return sendNftUtxoSelection(nftTokenId, amount, slpAppKit)
                .map {
                    val addrTo = SlpAddressFactory.create().getFromFormattedAddress(slpAppKit.wallet().params, toAddress).toCash()
                    // Add OP RETURN and receiver output
        ...
        fun sendNftUtxoSelection(nftTokenId: String, numTokens: Double, slpAppKit: SlpAppKit): Single<SendTokenUtxoSelection> {
            return Single.fromCallable {
                // Wrap for now to protect against blocking non reactive calls
                val tokenDetails: NonFungibleSlpToken = slpAppKit.getNft(nftTokenId)
                val sendTokensRaw = toRawAmount(numTokens.toBigDecimal(), tokenDetails)
                var sendSatoshi = DUST_LIMIT // At least one dust limit output to the token receiver
                ...
                var inputTokensRaw = ULong.MIN_VALUE
                var inputSatoshi = 0L
                val selectedUtxos = slpAppKit.nftUtxos
                    .asSequence()
                    .filter { it.tokenId == nftTokenId }
                    .sortedBy { it.tokenAmountRaw }
                    .takeWhile {
                        val amountTooLow = inputTokensRaw < sendTokensRaw
                        if (amountTooLow) {
                ...
                SendTokenUtxoSelection(nftTokenId, quantities, changeSatoshi, selectedUtxos)
            }
        }

        fun sendNftUtxoSelection(nftTokenId: String, numTokens: Double, slpAppKit: SlpBIP47AppKit): Single<SendTokenUtxoSelection> {
            return Single.fromCallable {
                // Wrap for now to protect against blocking non reactive calls
                val tokenDetails: NonFungibleSlpToken = slpAppKit.getNft(nftTokenId)
                val sendTokensRaw = toRawAmount(numTokens.toBigDecimal(), tokenDetails)
